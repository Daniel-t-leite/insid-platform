...
def gerenciar_modos_falha_anomalias():
    ...
    # 1. SELEÇÃO DE MODO DE FALHA (APENAS SE NÃO ESTIVER EDITANDO)
    if not st.session_state.get('editando_modo_falha'):
        with st.container():
    ...
    if st.session_state.modo_falha_selecionado and not st.session_state.get('editando_modo_falha'):
        with st.container():
            ...
            novos_tipos_barragem = st.multiselect(
                "🏗️ Tipos de Barragem Associados (desmarque para remover):",
        ...
        if submodo == "Visualizar":
            ...
            if not df_modos.empty:
                ...
                modo_selecionado = st.selectbox(
                    "Selecione um modo de falha para cadastrar anomalias:",
                    options=df_modos['id'],
                    format_func=lambda x: f"{df_modos[df_modos['id']==x]['nome'].values[0]}"
                )
                
                if st.button("Cadastrar Anomalias para este Modo"):
                    st.session_state.modo_falha_selecionado = modo_selecionado
                    st.session_state.editando_anomalia = None
                    st.session_state.editando_modo_falha = False
                    st.rerun()
            else:
...
def obter_tipos_zonas_bar(barragem_id: int, usuario_id: int) -> List[Tuple]:
    """
    Obtém todas as zonas cadastradas para uma barragem específica
    ...
    Returns:
        Lista de tuplas com (id, nome, descricao, material_id, nome_material)
    """
    conn = None
    try:
        conn = conectar_db()
        ...
        cursor.execute("""
            ...
            FROM 
                zonas_barragem zb
            JOIN 
                tipos_materiais tm ON zb.material_id = tm.id
            WHERE 
                zb.barragem_id = ? AND zb.usuario_id = ?
            ORDER BY 
                zb.nome
        """, (barragem_id, usuario_id))
...
## ------------------------------------------
## PÁGINA PRINCIPAL COM GERENCIAMENTO DE ANOMALIAS
## ------------------------------------------
...
def pagina_principal():
    ...
    if barragens_usuario:
        ...
        if barragem_analise:
            ...
            # SEÇÃO DE GERENCIAMENTO DE ANOMALIAS
            ...
            with tab_listar:
                ...
                if anomalias:
                    for anomalia in anomalias:
                        with st.expander(f"Anomalia #{anomalia[0]} - {anomalia[3]}"):  # ID e Tipo
                            cols = st.columns([1, 3])
                            with cols[0]:
                                if anomalia[9]:  # image_path
                            ...
                            with cols[1]:
                                ...
                                st.write("**Fontes de detecção:**", " | ".join(fontes) if fontes else "Nenhuma fonte especificada")
                else:
            ...
            with tab_adicionar:
                ...
                with st.form("form_adicionar_anomalia", clear_on_submit=True):
                    ...
                    tipos_anomalia = obter_tipos_anomalia(st.session_state['usuario_id'])
                    zonas_barragem = obter_tipos_zonas_bar(barragem_analise[0], st.session_state['usuario_id'])
                    materiais = obter_tipos_materiais(st.session_state['usuario_id'])
                    
                    # Campos do formulário
                    tipo_selecionado = st.selectbox(
                        "Tipo de Anomalia*",
                        options=tipos_anomalia,
                        format_func=lambda x: x[1],
                        help="Selecione o tipo de anomalia observada"
                    )
                    
                    zona_selecionada = st.selectbox(
                        "Zona da Barragem*",
                        options=zonas_barragem,
                        format_func=lambda x: f"{x[1]} ({x[4]})",
                        help="Selecione a zona da barragem onde a anomalia foi observada"
                    )
                    ...
                    material_selecionado = st.selectbox(
                        ...
                        options=materiais,
                        format_func=lambda x: x[1],
                        help="Selecione o material relacionado à anomalia"
                    )
                    
                    descricao = st.text_area(
                        "Descrição Detalhada*",
                        help="Descreva com detalhes a anomalia observada"
                    )
                    
                    data_observacao = st.date_input(
                        "Data de Observação*",
                        help="Data em que a anomalia foi observada"
                    )
                    
                    image_path = st.file_uploader(
                        "Imagem da Anomalia (opcional)",
                        type=['png', 'jpg', 'jpeg'],
                        help="Faça upload de uma imagem da anomalia, se disponível"
                    )
                    ...
                    if st.form_submit_button("✅ Registrar Anomalia"):
                        if not descricao:
                            ...
                            # Processar upload de imagem (simplificado)
                            img_path_str = None
                            if image_path:
                                # Aqui você implementaria o salvamento do arquivo
                                img_path_str = f"uploads/anomalia_{barragem_analise[0]}_{int(time.time())}.{image_path.name.split('.')[-1]}"
            ...
            with tab_editar:
                ...
                if anomalias:
                    anomalia_selecionada = st.selectbox(
                        "Selecione a anomalia para edição",
                        options=anomalias,
                        format_func=lambda x: f"Anomalia #{x[0]} - {x[3]} ({x[14]})"
                    )
                    ...
                    if anomalia_selecionada:
                        with st.form("form_editar_anomalia"):
                            ...
                            # Encontra os valores atuais
                            tipo_atual = next((t for t in tipos_anomalia if t[0] == anomalia_selecionada[2]), None)
                            zona_atual = next((z for z in zonas_barragem if z[0] == anomalia_selecionada[4]), None)
                            material_atual = next((m for m in materiais if m[0] == anomalia_selecionada[6]), None)
                            ...
                            novo_tipo = st.selectbox(
                                ...
                                options=tipos_anomalia, 
                                format_func=lambda x: x[1],
                                index=tipos_anomalia.index(tipo_atual) if tipo_atual else 0
                            )
                            nova_zona = st.selectbox(
                                "Zona da Barragem", 
                                options=zonas_barragem, 
                                format_func=lambda x: f"{x[1]} ({x[4]})",
                                index=zonas_barragem.index(zona_atual) if zona_atual else 0
                            )
                            novo_material = st.selectbox(
                                "Material", 
                                options=materiais, 
                                format_func=lambda x: x[1],
                                index=materiais.index(material_atual) if material_atual else 0
                            )
                            nova_descricao = st.text_area("Descrição detalhada", value=anomalia_selecionada[8])
                            nova_data = st.date_input("Data de Observação", value=datetime.strptime(anomalia_selecionada[14], '%Y-%m-%d').date())
                            ...
                            with col1:
                                if st.form_submit_button("💾 Salvar Alterações"):
                                    ...
                                    st.warning("Funcionalidade de edição será implementada na próxima versão")
                            with col2:
                                if st.form_submit_button("🗑️ Excluir Anomalia"):
                                    # Implementar função de exclusão
                                    st.warning("Funcionalidade de exclusão será implementada na próxima versão")
                else:
                    st.info("Nenhuma anomalia registrada para esta barragem")
    else:
        st.warning("Você ainda não cadastrou nenhuma barragem")

# Nota: Você precisará implementar as funções auxiliares restantes:
# - obter_barragens_usuario()
# - obter_barragem_analise()
# - definir_barragem_analise()
# - editar_barragem()
# - atualizar_anomalia()
# - excluir_anomalia()
